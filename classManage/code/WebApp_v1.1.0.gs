/**
 * WebApp_v1.1.0.gs - Enhanced Web App with tc-api Integration
 * 
 * Version: v1.1.0
 * Update Date: 2025-11-29
 * Updates: Added teacher-class selection, enhanced attendance, tc-api integration
 * 
...
function doGet(e) {
  try {
    ...
    switch (page) {
      ...
      case 'sync':
        return renderSync(user);  // NEW: Sync page
      default:
        return renderDashboard(user);
    }
  } catch (error) {
...
function getDashboardData() {
  try {
    ...
    const attendanceStats = getAttendanceStatsByClass();  // NEW
    ...
    const data = {
      ...
      attendanceStats: attendanceStats,  // NEW
...
/**
 * NEW: Get attendance statistics by class
 */
...
/**
 * NEW: Get classes list with optional grade filter
 */
...
/**
 * NEW: Get students by class for attendance
 */
...
function getSystemStatus() {
  return {
    ...
    lastSync: getSettingValue('last_tc_api_sync') || 'Never'
  };
}
